...
# FUNCTION BODY # 
  ...
  if (verbose){
    ...
    if(length(unique_1) != 0){
      for(strand in 1:(length(unique_1)/2)){
        ...
        cat(paste("\t[", toString(strand), "] ", st, " (Length: ", ln, ")\n", sep=""))
      }
    }
    else
      cat("\tNone\n")
  ...
  cat(paste("Unique Genomic Sequences in Genome 2: ", length(unique_2)/2, "\n", sep=""))
  cat(paste("\nTotal Inversions: ", invert_count, "\n"))
  cat(paste("Total Rearrangements: ", block_count, "\n"))
  cat(paste("Total Ins/Del/Transfer: ", length(unique_1)/2 + length(unique_2)/2, "\n\n", sep=""))
