...
##########
GRarr_v2 <- function(synt, min_unique_length = 0, verbose = FALSE, actual=c(), test_run=1){
...
# END ERROR CHECKING------------------------------------------------------------
  ...
  # ===== Inversions =====
  inversions <- function(graph, begin, AA){
    ret_lst <- vector("list", length=nrow(graph))
    ...
    while (!done){ #inversions can create oriented gray edges, so we need to recheck all vertices if we make an inversion
      ...
      test_vertex <- begin
      seen <- vector("numeric", length=nrow(graph))
      ind <- 1
      gray <- FALSE
      
      while(!(test_vertex %in% seen)){ #iterating over graph vertices looking for oriented gray lines
        seen[ind] <- test_vertex
        ind <- ind + 1
        gray <- !gray
        ...
        if(AA && (test_vertex==nrow(graph) || test_vertex==(nrow(graph) - 1))){
          if (gray)
            test_vertex <- graph[test_vertex, 2]
          else
            test_vertex <- graph[test_vertex, 1]
          next
        }
        
        if(graph[test_vertex,3] + graph[graph[test_vertex,2],3] == 0){ #if we find one, set done to FALSE and perform DCJ
          if(AA && (graph[test_vertex,2] == nrow(graph) || graph[test_vertex,2] == (nrow(graph)-1))){
            if (gray)
              test_vertex <- graph[test_vertex, 2]
            else
              test_vertex <- graph[test_vertex, 1]
          ...
          #2-cycles are always given priority for inversions (as they resolve two breakpoints)
          for(vert in c(test_vertex, graph[test_vertex,2], graph[test_vertex, 1], graph[graph[test_vertex,2],1])){
            adj <- vert + 2*(vert%%2) - 1 #other end of synteny block
            adj_gray <- graph[adj, 2]
            adj_bl <- graph[adj, 1]
            adj_gray_bl <- graph[graph[adj, 2], 1]
            
            if (graph[adj, 3] + graph[adj_gray, 3] == 0 && #if other end has an oriented gray
                graph[adj_bl,2] == adj_gray_bl && #and the black vertices are connected by gray
                graph[adj_bl, 3] + graph[adj_gray_bl, 3] == 0) { #and this gray is oriented
              test_vertex <- adj #then use this vertex instead
              break()
            }
          }
          if(test_vertex == begin || graph[test_vertex,2] == begin)
            begin <- graph[graph[test_vertex,2], 1]
          graph <- DCJ(graph, test_vertex, graph[test_vertex,2]) #DCJ operation is performed on the two vertices incident to the gray line
          ...
          if (gray)
            test_vertex <- graph[cur, 2]
          else
            test_vertex <- graph[cur, 1]
          ret_lst <- lappend(ret_lst, paste("inversion: ", paste(graph_to_genome(graph, AA), collapse=" ")))
    ...
    return(list("graph"=graph, "count"=invert_count, "ops"= ret_lst))
  }
  # ==== end inversions ====
...
# FUNCTION BODY # 
  ...
  # ===== Inversions =====
  inv <- inversions(graph, nrow(graph), AA)
  rearrangements <- lappend(rearrangements, inv$ops)
  invert_count <- inv$count
  graph <- inv$graph
  
  tmp <- 1 + length(inv$ops)
  
  if(AA){
    inv <- inversions(graph, nrow(graph)-2, AA)
  }
  else{
    inv <- inversions(graph, nrow(graph)-1, AA)
  }
  
  invert_count <- invert_count + inv$count
  graph <- inv$graph
  rearrangements <- lappend(rearrangements, inv$ops)
  gen <- graph_to_genome(graph, AA)
  
  while(sum(gen < 0) >= 1){
    for(val in which(gen<0)){
      start_val <- abs(2*gen[val]) - 1
      inv <- inversions(graph, start_val, AA)
      invert_count <- invert_count + inv$count
      graph <- inv$graph
      rearrangements <- lappend(rearrangements, inv$ops)
      gen <- graph_to_genome(graph, AA)
  ...
  rearrangements <- rearrangements[-which(lapply(rearrangements,is.null) == TRUE)]
  # ===== end of inversions =====
