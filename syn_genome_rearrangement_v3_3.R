...
##########
GRarr_v3 <- function(synt, opp_value = 0, min_unique_length = 0, verbose = FALSE, actual=c(), test_run=0, first_gen=1, second_gen=2){
...
# FUNCTION BODY # 
  ...
  while(!done){
    ...
    for(i in order){
      ...
      #block interchange====
      if((gen[i] + 1) != gen[i+1] && ((gen[i]+1) %in% gen[i:len])
         || (gen[i] - 1) != gen[i-1] && ((gen[i]-1) %in% gen[1:(i-1)])){
