...
##########
GRarr_v3 <- function(synt, num_runs=-1, verbose = FALSE, mean = FALSE, #main user parameters
                     opp_value = 0, min_unique_length = 0, #extra user parameters
                     actual=c(), test_run=0, first_gen=1, second_gen=2){ #internal parameters
...
# ERROR CHECKING  --------------------------------------------------------------
  ...
  if(nrow(synt[[second_gen,first_gen]]) == 1){
    len_1 <- syn[[1,1]]
    len_2 <- syn[[2,2]]
    start_1 <- synt[[2,1]][,5]
    start_2 <- synt[[2,1]][,6]
...
# FUNCTION BODY # 
  ...
  synblocks <- synt[[second_gen,first_gen]] #this grabs the information on the blocks
